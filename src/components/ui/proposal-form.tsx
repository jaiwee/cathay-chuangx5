...
function getFlightDurationMinutes(departureTime: string, arrivalTime: string): number {
    ...
    async function fetchActivities() {
      try {
        ...
        if (data && Array.isArray(data)) {
          ...
          const hasAllTimes = mappedActivities.every(activity => activity.start_time);
          if (!hasAllTimes && flightData) {
            const departureTime = formatTimeFromDB(flightData.departure_time);
            const calculatedActivities = calculateActivityTimes(mappedActivities, departureTime);
            console.log("Calculated activities with times:", calculatedActivities);
    ...
    <div className="w-full h-full bg-white flex flex-col overflow-hidden px-6 py-4 relative min-w-0">
      ...
      {isLoading && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white bg-opacity-90 backdrop-blur-sm">
          <div className="flex flex-col items-center space-y-4">
            ...
            <p className="text-sm font-normal text-gray-800">Loading flight itinerary...</p>
          </div>
        </div>
      )}
      <div className="flex-shrink-0">
        <h1 className="mb-4 text-3xl font-bold text-gray-800">Flight Itinerary</h1>
        ...
        <div className="mb-2 space-y-1 text-sm font-normal text-gray-800">
          {isLoadingFlight ? (
            ...
            <>
              <h2 className="text-xl text-gray-800 break-words">{formatDateFromDB(flightData.departure_time)}</h2>
            </>
          ) : (
            ...
            <div className="relative flex flex-1 items-center px-6">
              ...
              <div className="relative flex-1 mx-4">
                ...
                <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-full pb-2 text-sm font-normal text-gray-800 whitespace-nowrap">
                  {formatDuration(flightData.duration || getFlightDurationMinutes(formatTimeFromDB(flightData.departure_time), formatTimeFromDB(flightData.arrival_time)))}
                </div>
      ...
      <div className="flex-1 min-h-0 mt-4 overflow-hidden">
        <ScrollArea className="h-full w-full">
          {isLoadingActivities ? (
            <div className="p-4 text-sm text-gray-600">Loading activities...</div>
          ) : activities.length === 0 ? (
      ...
      <div className="flex-shrink-0 mt-8 flex justify-end">
        <Button
          ...
          onClick={() => {
            ...
            const flightDuration = flightData.duration || getFlightDurationMinutes(formatTimeFromDB(flightData.departure_time), formatTimeFromDB(flightData.arrival_time));
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-8 max-w-md mx-4 flex flex-col items-center space-y-4 pointer-events-auto">
            {!showGenerating ? (
              <>
                ...
                <p className="text-xl font-bold text-gray-800">Submission Success</p>
              ...
              <>
                ...
                <p className="text-xl font-bold text-gray-800">Generating Smart Recommendations...</p>
                <p className="text-sm font-normal text-gray-800 text-center leading-relaxed">
                  Please wait while we create personalized recommendations for your post-flight journey.
                </p>
              </>
            )}
          </div>
